...
export function DietMealTimelineItem({
  meal,
  status,
  active,
  onPress,
  isLast,
}: DietMealTimelineItemProps) {
    ...
    >
      ...
      <View className="flex-1">
        ...
        <AppText className="mt-0.5 text-sm text-text-muted">
          {meal.time} • {Math.round(total.calories)} kcal •{" "}
      ...
      {active && (
        <View className="bg-brand-primary/10 px-3 py-1 rounded-full border border-brand-primary/20">
          <AppText className="text-[10px] font-bold uppercase tracking-widest text-brand-secondary">
            Agora
          </AppText>
        </View>
      )}
